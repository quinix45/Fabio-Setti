...
ere I just touched up the plot a bit and set the bins to 15. It is much easier to tell what the distribution looks like now.
...
The `lm()` function is what runs [ordinary least squares (OLS) regression](https://en.wikipedia.org/wiki/Ordinary_least_squares){target='_blank'} in `R`. The function stands fro "linear model"!
...
Equivalently, we can also check that the residuals residuals are evenly distributed around 0 and there are no non-linear trends.  
