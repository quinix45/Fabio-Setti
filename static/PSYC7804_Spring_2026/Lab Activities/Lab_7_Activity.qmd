...
**3.** Find the the turning point of the quadratic line. Then compute a $95\%$ bootstrapped confidence interval for it. Bootstrap the regression results twice with 2 different seeds and compare the bootstrapped results.
...
boot_quad1 <- car::Boot(quad_reg, R = 1000)

top_boot1 <- -boot_quad1$t[,2]/(2*boot_quad1$t[,3])

quantile(top_boot1, c(.025, .975))
...
boot_quad2 <- car::Boot(quad_reg, R = 1000)

top_boot2 <- -boot_quad2$t[,2]/(2*boot_quad2$t[,3])

quantile(top_boot2, c(.025, .975))

```

- Plot histograms of the two different bootstrapped turning points. Do you notice anything strange? 
...
**4.** You should notice that something a bit strange is happening when we bootstrap our quadratic regression. Plot your quadratic regression. Does anything stand out to you? After looking at the plot, do you see any potential problems with our quadratic regression?
