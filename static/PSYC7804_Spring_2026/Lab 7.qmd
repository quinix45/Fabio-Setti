...
We can do some [string manipulation](https://www.geeksforgeeks.org/string-manipulation-in-r/){target="_blank"} to remove the last 6 characters!
...
It is interesting to note that dates in `R` are interpreted as the *days* that have passed since the [Unix Epoch](https://en.wikipedia.org/wiki/Unix_time){target="_blank"}, January 1<sup>st</sup>, 1970. This is how most software/computers keep track of time 🤓
...
So `2016-01-01` is 46 years after January 1<sup>st</sup>, 1970. This fact is useful to keep in the back of your mind if your mind if you are working with dates, as you will often find yourself needing to apply numerical operations to them.
...
![✨ clean data ✨](Additional%20files/Images/pacha_meme.png){width=40%}
...
labels <- c("Day of measurement",
            "Temperature in Celsius",
            "Temperature in Fahrenheit")

labels_react <- reactbl_col.desc(names = colnames(Temp_avg_2021),
                                 desc =  labels,
                                 style = "cursor: pointer; text-decoration: none;",
                                 info_color = "#7a0b80")

reactable::reactable(cbind(Temp_avg_2021[,1], round(Temp_avg_2021[,2:3],2)),
...
```
 ...
 We will not be able to interpret our regression coefficients (the linear term specifically) with such multicollinearity, as discussed in [Lab 6](https://fabio-setti.netlify.app/psyc7804_spring_2026/lab%206#/title-slide){target="_blank"}
:::
...
And the multicollinearity problem is solved 🪄
...
**3.** The direction of the bend. If the sign of $b_2$ is negative, the curve will be a bell (as in our case), if the sign is negative, it will be a U.
:::
...
The 3D visualization of a piecewise regression is kinda funny:
