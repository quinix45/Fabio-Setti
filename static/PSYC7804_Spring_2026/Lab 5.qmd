---
title: "Lab 5: Semi-partial, Partial-correlations, and Model Comparison"
author: "Fabio Setti"
...
format:
   revealjs:
      footer: "PSYC 7804 - Lab 5: Semi-partial, Partial-correlations, and Model Comparison"
      width: 1280
      height: 720
      chalkboard: true
      slide-number: c/t 
...
So, $r_{xy.z}-.16$ is the correlation between `happiness` and `age` after removing the proportion of variance explained by `friends` from both variables. 
...
- $r_{y(x.z)} = -.15$ represents the relationship between $y$ and $x$ after the removing the variance explained by $z$ from $x$.  

- $r_{x(y.z)} = -.16$ represents the relationship between $x$ and $y$ after removing  the variance explained by $z$ from $y$.  
...
$$AIC = N \times ln(\frac{SS_{\mathrm{residuals}}}{N}) + 2p$$ 
...
$$BIC =  N \times ln(\frac{SS_{\mathrm{residuals}}}{N}) + p\times ln(N)$$

:::

::: {.fragment fragment-index=3}

The $N \times ln(\frac{SS_{\mathrm{residuals}}}{N})$ part, which is shared by both formulas, measures model fit. In contrast $2p$ is the AIC penalty, while $p\times ln(N)$ is the BIC penalty. The BIC penalty is stricter.
:::
