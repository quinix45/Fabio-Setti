---
title: "Lab 9: Categorical Predictors"
author: "Fabio Setti"
...
format:
   revealjs:
      footer: "PSYC 7804 - Lab 9: Categorical Predictors"
      width: 1280
      height: 720
      chalkboard: true
      slide-number: c/t 
...
hsb2 <- rio::import("https://fabio-setti.netlify.app/data/hsb2.csv")
```
...
::: {.fragment fragment-index=3}
<div style="font-size: 24px"> Remember that in regression, the intercept is *expected mean value* of $Y$ when $X = 0$, and the slope is the *expected mean change* in $Y$ per unit increase in $X$. </div>
:::
...
There are no coincidences in statistics. If two methods give some of the same results, they must be related in some way. When you see such patterns, ask yourself *why* two methods produce the same results. Once you answer that question, you will have gained lots of insight!
:::
:::
...
::: {.fragment fragment-index=2}
<div style="font-size: 24px"> The $R^2$ is the exact same as the regression from 2 slides ago; whether we code `male` and `female` as $0$ and $1$ or $-314$ and $-140$ (some random numbers I chose), *the models are equivalent*. However, the values of the intercept and slope are now meaningless. </div>
:::
...
<div style="font-size: 22px; padding-top: 24px;"> **Note:** We will go over some popular coding schemes, but you may devise your own depending on your hypotheses. </div>
