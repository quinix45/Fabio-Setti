...
<div style="font-size: 22px"> We'll continue with the `hsb2` dataset from [Lab 9](https://fabio-setti.netlify.app/PSYC7804_Spring_2026/Lab%209.html){target="_blank"}. 
 </div>
...
In Lab 9 we looked at categorical predictors and a bunch of coding schemes for categorical variables, but we will stick to [dummy coding](https://fabio-setti.netlify.app/psyc7804_spring_2026/lab%209#/dummy-coding){target="_blank"} for this lab.
...
First let's start with a regression with the `gender` only predicting `write` score. We saw this in Lab 9 already.
...
When we have a categorical moderator, interpreting the interaction terms boils down to looking at how the slopes of the groups differ.
