...
The `dominanceanalysis` package [@Navarrete_Soares_2024] contains functions to run dominance analysis in R.
...
labels <- c("Intense Music Preference (Standardized)",
            "Adventurousness",
            "Intellect",
            "Unconventionality")

labels_react <- reactbl_col.desc(names = colnames(music),
                                 desc =  labels,
                                 style = "cursor: pointer; text-decoration: none;",
                                 info_color = "#7a0b80")

reactable::reactable(round(music, 2),
...
::: {.fragment fragment-index=3}
<div style="font-size: 24px;">  But wait, the relation between `Advnt` and `Intense` becomes negative after accounting for `Intel` 🙀 </div>
:::
...
nice_3D_plot(y = music$Intense,
             x1 = music$Advnt,
             x2 = music$Intel,
             axis_names = c("Intense", "Adventurousness", "Intellect"),
             reg_plane = TRUE) %>%  
...
If you remember, [semi-partial correlations](https://raw.githack.com/quinix45/PSYC-7804-Regression-Lab-Slides/main/Slides%20Files/Lab%206.html#/semi-partial-correlation-by-hand){target="_blank"} calculate the correlation between $y$ and $x$ after removing the explained variance of $z$ from only one variable.
...
As shown in [Lab 4](https://fabio-setti.netlify.app/psyc7804_spring_2026/lab%204#/title-slide){target="_blank"}, regression coefficients are calculated based on the residuals of the predictors after controlling for all other predictors. But, if `Advnt` and `Intel` are so correlated, what happens to their residual after controlling fo one another? 
...
::: {.fragment fragment-index=2}
<div style="font-size: 24px"> The residuals have almost no variance left! If a variable has variance of 0, it is always the same number (i.e., a constant), and cannot predict anything. A consequence of multicollinearity is that the regression coefficients that are calculated based on these residuals **should not be interpreted**. 
 </div>
:::
...
<center>
<figure>
  <img src="Additional files/Images/David_Budescu.jpg">
...
::: {.fragment fragment-index=3}
![](Additional files//Images/type-i-and-ii-error-2.png)
:::
...
::: {.fragment fragment-index=2}
<div style="font-size: 24px; padding-bottom: 18px;"> The one thing to keep in mind is that the smaller $\mathrm{SE}_{b_1}$, the higher your power. $n$, the sample size, is in the denominator, meaning that larger sample size decreases $\mathrm{SE}_{b_1}$...so, more $n$, more power 😀. </div>
:::
...
<div style="font-size: 24px"> So, I think people care about power for the wrong reasons 🤷 Some related blog posts/manuscripts by Andrew Gelman:
