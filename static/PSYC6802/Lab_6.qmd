...
So, `education` explained $72\%$ of the varaince in `presrtige`, while `women` explained $19\%$ of the varaince in `income`.  For any `lm` object, the $R^2$ along fit its significance test is also at the end of the `summary()` output.  
...
Here is a visualization of our *best possible line* before and after finding out about `education`. The concept is the same as before, but visulized in a slightly different way:
...
You should see that all the red dotted lines are much shorter on average. That means that the *variance of the residuals is much smaller*. Once again, this is the *varaince explained*.
