...
If instead of the mean of `SAT_Math` alone we use the mean of `SAT_Math` by each `Borough` our prediction, we cen reduce the residual variance:
...
To calculate $\eta^2$ we use the `eta_saqaured()` function from the `effectsize()` package:
...
normal ANOVAs also assume that the variance is equal across groups. Just like the *t*-test, the assumption of equal variances never holds in practice.
