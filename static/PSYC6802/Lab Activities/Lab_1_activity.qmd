...
Lab activities are usually much shorter than this. You are not expected to turn in this lab activity, but I do encourage you to attempt solving the questions below to better familiarize yourself with `R`.
...
**HINT:** You can apply mathematical operations to vectors of the same length. Mathematical operations will be performed between the respective elements of each vector. 
...
sum(x)/length(x)
```
**5.** Calculate the *standard deviation* of all of the numbers contained in the **even_10**, **odd_10**, and **mult_10** (so only 1 standard deviation, not 3). You will also need to find the function that calculates the standard deviation.
...
**HINT:** here you need to use "()" to tell R the correct order of operations and functions. 
...
**More Practice questions on the next page**
...
**6.** Click [here](https://fabio-setti.netlify.app/data/Mammal_Sleep.csv){target="_blank"} to download the **Mammal_Sleep.csv** file, import it into R, and name it `dat`. You can find the description of the variables in the data set here (https://www.openintro.org/data/index.php?data=mammals). Additionally, there is an extra variable, *primate*, that specifies whether the mammal is a primate or not. Explore the data either visually or with the `str()` function to get a better sense of what you are looking at!

```{r}
# make sure you have the data in the same working directory. For lab activity I create a folder named "Lab_Activity_Data" where I store all the data I will need for lab activities (If this is confusing come ask me to explain it!). 

library(rio)

dat <- import("Lab_Activity_Data/Mammal_Sleep.csv")
```
...
**HINT:** In the lecture we looked at a function for counting elements.
