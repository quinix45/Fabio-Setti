...
## Factor Variables{visibility="hidden"}

<!-- hiding this for now because it does not make sense to introduce factor variables now -->

On the previous slide we saw that both `country_name` and `Region` are character variables. R is a bit peculiar because, aside from character variables, it also has `factor` variables. 

:::: {.columns}
::: {.column width="50%"}


<div style="font-size: 26px"> Given a character vector with some repeating values: </div>
...
char_EX <- c("control", "control", "treatment", "control", "treatment")
str(char_EX)
```

<div style="font-size: 26px"> We can turn the character variable into a factor variable: </div>
...
In genreal, it may be a good idea to turn character variables into factors (although I don't do it here)
